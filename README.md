...
The vendor arrangement behind the websites lab. That lab used a paid AI site builder with a promo code from a per-institution deal, which cannot be handed on. The kit's version of the lab is tool-agnostic and the External Tools chapter covers choosing one. Everything else about the meeting is intact.
...
I built this course, and still maintain it, in conversation with an AI assistant: the meetings, the activities, the website, and this kit. The setup in `AGENTS.md` and `.claude/skills/` exists so you can continue that way in your own repository rather than inheriting a frozen document.
